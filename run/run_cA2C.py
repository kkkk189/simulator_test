...
for n_iter in np.arange(25):
    ...
    for ii in np.arange(EP_LEN + 1):
        ...
        # action_tuple: is the re-allocation of all the drivers (n_valid * (action_dim - 1) x 3)
        #               (start_node_id, end_node_id, num_driver)
        # valid_action_prob_mat: action probability for state value function (n_valid x action_dim)
        # policy_state: is the state of each agent x valid_action_dim (n_valid * valid_action_dim x state_dim)
        # action_choosen_mat:   is the action (one-hot encoding) of each agent x valid_action_dim
        #                       (n_valid * valid_action_dim x action_dim)
        ...
        # r0: Takes the valid node rewards and divides them by the number of drivers
        ...
        if ii != 0:
            ...
            # Stores s, a, V_target, s'
            replay.add(state_mat_prev, action_mat_prev, targets_batch, s_grid)
            # Stores J, a, A, C (n_valid_grid x action dim (7))
