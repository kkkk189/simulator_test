...
class Estimator:
    ...
    def _build_value_model(self):
        ...
        # State value function (n_valid x 1)
    ...
    def _build_mlp_policy(self):
        ...
        # P logits (n_valid x action dim)
        self.logits = logits = fc(l3, "logits", self.action_dim, act=tf.nn.relu) + 1  # avoid valid_logits are all zeros
        # Q valid (n_valid x action dim)
        self.valid_logits = logits * self.neighbor_mask

        # Converting the logits to softmax probability (n_valid x action dim) (Action Probability - used to choose action)
    ...
    def action(self, s, context, epsilon):
        ...
        # Computes the centralized state-value, v_t
        ...
        # Provides the indices of each row with 1 hot_code (1 x n_valid_grid)
        grid_ids = np.argmax(s[:, -self.n_valid_grid:], axis=1)

        # compute neighbor mask according to centralized value (n_valid_grid x action dim (7))
        ...
        # compute policy probability. (n_valid x action_dim)
        ...
        for idx, grid_valid_idx in enumerate(grid_ids):
            ...
            # cast invalid action to zero, valid numerical issue.
            action_prob[self.valid_action_mask[grid_valid_idx] == 0] = 0

            valid_prob.append(action_prob)   # action probability for state value function (1 x action_dim)
            if int(context[idx]) == 0:
                continue

            # Chooses an action from action_dim, for all drivers in that grid, based on probability p
            ...
            # Start node is the current grid ID
            start_node_id = self.env.target_grids[grid_valid_idx]
            for curr_action_idx, num_driver in enumerate(curr_action_indices):
                if num_driver > 0:
                    # Gets the node_ID (could also be an invalid grid) of the neighbor_node where the drivers are supposed to go
                    end_node_id = int(self.valid_neighbor_node_id[grid_valid_idx, curr_action_idx])
                    # Stores the movement of drivers if they don't remain in the same grid
                    if end_node_id != start_node_id:
                        action_tuple.append((start_node_id, end_node_id, num_driver))

                    # book keeping for training
                    temp_a = np.zeros(self.action_dim)  # [0,0,0,0,0,0,0]
                    temp_a[curr_action_idx] = 1         # [1,0,0,0,0,0,0]
                    action_choosen_mat.append(temp_a)
                    policy_state.append(s[idx]) # agent idx from s (agent states) (1 x n_valid * 3 + T)
    ...
    def update_value(self, s, y, learning_rate, global_step):
        """
        Updates the estimator towards the given targets. (For Critic)
